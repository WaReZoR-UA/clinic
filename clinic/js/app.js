...
let slider_about = new Swiper('.news__images', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
...
if (spollersArray.length > 0) {
  ...
  function hideSpollersBody(spollersBlock) {
    ...
    // console.log('spollerActiveTitle', spollerActiveTitle);
...
let _slideUp = (target, duration = 500) => {
  if (!target.classList.contains("_slide")) {
    // Показывает картинку
    target.parentNode
      .querySelector(".spoller__image")
      .classList.toggle("_active");

    // Меняем кнопку
    const button = target.parentNode.querySelector(".spoller__button");
    button.classList.toggle("_icon-arrow_u");
    button.innerHTML = "Детальнiше";
